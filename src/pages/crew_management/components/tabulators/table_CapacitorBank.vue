    ...
    <div class="content-section q-mt-md">
      ...
      <div class="feeder-list-container">
        <div v-for="feeder in feeders" :key="feeder.id"
          ...
          <div class="text-h6 text-grey-9">CB - {{ feeder.capacitorBankNo || feeder.id }}</div>
          <q-icon name="play_arrow" size="sm" color="grey-7" />
        </div>
      </div>

      <!-- Add Feeder Button: Cyan Rounded -->
      <div v-if="feeders.length < 6" class="flex justify-center q-mt-md">
        <q-btn label="Add Capacitor Bank" unelevated class="survey-add-btn" @click="openAddDialog" />
      </div>
    </div>
    ...
    <q-dialog v-model="feederDialogOpen" :maximized="$q.screen.lt.md" transition-show="slide-up"
      ...
      <q-card class="survey-dialog-card" :style="$q.screen.gt.sm ? 'width: 70vw; max-width: 900px;' : ''">
        <q-toolbar class="bg-indigo-10 text-white">
          <q-toolbar-title class="text-subtitle1">{{ editingIndex === -1 ? 'Add New' : 'Edit' }} Capacitor Bank
          ...
          <div class="row q-col-gutter-md">
            ...
            <div class="col-12 col-sm-6">
               <div class="text-subtitle2 q-mb-sm"
                ...
                <q-input dense outlined v-model="form.ratingPerBank" placeholder="Enter"
                  :class="$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light'" />
            </div>
    ...
    <q-dialog v-model="deleteConfirmOpen">
      <q-card>
        <q-card-section class="row items-center">
          ...
          <span class="q-ml-sm">Are you sure you want to delete this CB?</span>
        </q-card-section>
...
const regulatoryControlOptions = ref([
  { label: 'Auto', value: 'auto' },
  { label: 'Manual', value: 'manual' },
  
])

const capacitorBankOptions = ref([
