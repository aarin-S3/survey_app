<template>
  <q-dialog v-model="internalModel" seamless @before-show="resetPosition" persistent>

    <q-card 
      :class="$q.dark.isActive ? 'bg-card-dark' : 'bg-card-light'"
      :style="[dialogStyle, { height: '521px', width: '700px !important', display: 'flex', flexDirection: 'column', boxShadow: $q.dark.isActive ? '1px 1px 70px 13px #000' : 'none', borderRadius: '2px' }]">
      ...
      <q-bar class="my-dialog text-white cursor-pointer" v-touch-pan.mouse="onPan"
        ...
        <div :class="$q.dark.isActive ? 'dialog-header-dark' : 'dialog-header-light'" style="font-size: 15px;">
          Create Crew Order
        </div>
        ...
        <q-btn dense flat icon="minimize" @click="onMinimize" class="q-mr-xs"
          ...
          <q-tooltip>Minimize to Footer</q-tooltip>
        </q-btn>
      ...
      <q-tabs v-model="tab" dense class="text-grey-7" active-color="grey" indicator-color="grey" align="left"
        style="height: 30px;" :style="$q.screen.lt.sm ? 'width: 50%;' : 'width: 30%;'">
        <q-tab name="notification_details" label="Details" />
        <q-tab name="customer_list" label="List" />
      </q-tabs>
      ...
      <q-scroll-area class="col q-pa-md"
        ...
        <q-tab-panels v-model="tab" animated class="bg-transparent text-white">
          ...
          <q-tab-panel name="notification_details" class="q-pa-none">
            <div class="row q-col-gutter-sm">
              <div class="col-12 col-md-6">
                <div class="text-subtitle2 q-mb-sm"
                  :class="$q.dark.isActive ? 'dialog-subtitle-dark' : 'dialog-subtitle-light'">Company Contact Info
                </div>
                <div class="row items-center q-mb-xs">
                  <div class="col-3 text-caption"
                    :class="$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light'">Name</div>
                  <div class="col-9"><q-input dense outlined v-model="form.name"
                      :class="$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light'" /></div>
                </div>
                <div class="row items-center q-mb-xs">
                  <div class="col-3 text-caption"
                    :class="$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light'">Phone</div>
                  <div class="col-9"><q-input dense outlined v-model="form.phone"
                      :class="$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light'" /></div>
                </div>
                <div class="row items-center q-mb-xs">
                  <div class="col-3 text-caption"
                    :class="$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light'">Email</div>
                  <div class="col-9"><q-input dense outlined v-model="form.email"
                      :class="$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light'" /></div>
                </div>
                <div class="row items-center q-mb-xs">
                  <div class="col-3 text-caption"
                  ...
                  <div class="col-9"><q-input dense outlined v-model="form.serviceCenter"
                      :class="$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light'" /></div>
                </div>
                <div class="row items-start q-mb-xs">
                  ...
                  <div class="col-9"><q-input dense outlined v-model="form.address"
                      :class="$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light'" /></div>
                </div>
              </div>

              <div class="col-12 col-md-6" :style="[$q.screen.lt.md ? 'padding-top: 0px;' : 'padding-top: 42px;']">
                <div class="row items-center q-mb-xs">
                  <div class="text-caption"
                    :class="[$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light',$q.screen.lt.md ? 'col-3' : 'col-4']">Creation Date
                  </div>
                ...
                <div class="row items-center q-mb-xs">
                  ...
                  <div class="" :class="[$q.screen.lt.md ? 'col-9' : 'col-8']"><q-checkbox class=""
                ...
                <div class="row items-center q-mb-xs" style="margin-bottom: 18px;">
                  <div class="text-caption"
                    :class="[$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light',$q.screen.lt.md ? 'col-3' : 'col-4']">Work Order ID
                  </div>
                  <div class="" :class="[$q.screen.lt.md ? 'col-9' : 'col-8']"><q-input dense outlined v-model="form.workOrderId"
                      :class="$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light'" /></div>
                </div>
                <div class="row items-center q-mb-xs" style="margin-bottom: 18px;">
                  <div class="text-caption"
                    :class="[$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light',$q.screen.lt.md ? 'col-3' : 'col-4']">Event Type</div>
                  <div class="" :class="[$q.screen.lt.md ? 'col-9' : 'col-8']"><q-select dense outlined v-model="form.eventType" :options="eventTypes"
                      :class="$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light'" />
                  </div>
                </div>
                <div class="row items-center q-mb-xs">
                  <div class="text-caption"
                    :class="[$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light',$q.screen.lt.md ? 'col-3' : 'col-4']">Outage</div>
                  <div class="" :class="[$q.screen.lt.md ? 'col-9' : 'col-8']">
                    <q-select dense outlined v-model="form.outageType" :options="outageTypes"
                      :class="$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light'" />
                  </div>
                </div>
              </div>
           ...
           <q-separator class="" />
            ...
            <div class="row q-col-gutter-md" style="margin-top: 1px;">
              <div class="col-12 col-md-6">
                <div class="text-subtitle2 q-mb-sm"
                  :class="$q.dark.isActive ? 'dialog-subtitle-dark' : 'dialog-subtitle-light'">Scheduled Event Date
                </div>
                <div class="row items-center q-mb-xs">
                  <div class="col-3 text-caption"
                ...
                <div class="row items-center">
                  <div class="col-3 text-caption"
                    :class="$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light'">Duration</div>
                  <div class="col-4"><q-input dense outlined v-model="form.schedDuration" mask="##:##"
                      placeholder="00:05"
                      :class="$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light'" /></div>
                </div>
              </div>
              ...
              <div class="col-12 col-md-6">
                <div class="text-subtitle2 q-mb-sm"
                  :class="$q.dark.isActive ? 'dialog-subtitle-dark' : 'dialog-subtitle-light'">Alternate Event Date
                </div>
                <div class="row items-center q-mb-xs">
                  <div class="col-3 text-caption"
                    :class="$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light'">Start Time</div>
                ...
                <div class="row items-center">
                  <div class="col-3 text-caption"
                    :class="$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light'">Duration</div>
                  <div class="col-4"><q-input dense outlined v-model="form.altDuration" mask="##:##" placeholder="00:05"
                      :class="$q.dark.isActive ? 'dialog-field-label-dark' : 'dialog-field-label-light'" /></div>
                </div>
              </div>
            </div>
          ...
          <q-tab-panel name="customer_list" class="q-pa-none">
            <div class="row">
...
const dialogId = 'crew-order-tabs-dia' // Unique ID for this dialog

// --- MINIMIZE LOGIC (UPDATED TO NEW STORE) ---
const onMinimize = () => {
  store.minimize(dialogId, 'Crew Order', () => {
    internalModel.value = true
  })
  internalModel.value = false
}

// --- DRAGGABLE & RE-CENTERING LOGIC ---
...
const tableData = Array.from({ length: 15 }, (_, i) => ({
  ...
  account: `ACC-${1000 + i}`,
  meter: `M-${5000 + i}`,
  address: `${100 + i} Main Street, Sector ${i + 1}`,
...
.input-field :deep(.q-field__control:not(:has(textarea))), .select-field :deep(.q-field__control) { height: 30px !important; }
:deep(.q-tab__label) { font-size: 11px; line-height: 1.715em; font-weight: 500; }
:deep(.q-tabs--dense .q-tab) { min-height: 30px !important; }
:deep(.q-field--auto-height).q-field--dense .q-field__control, :deep(.q-field--auto-height.q-field--dense) .q-field__native { min-height: 1.6rem; }
